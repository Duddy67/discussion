    ...
    <div class="card-header row">
        ...
        @if ($canEdit && $count == $key + 1)
            <div id="edit-comment-forms-{{ $key }}">
                ...
                <form id="updateComment-{{ $comment->id }}" action="{{ route('discussions.comments.update', ['comment' => $comment]) }}" style="display:none;" method="post">
                    @method('put')
                    @csrf
                ...
                <form id="deleteComment-{{ $comment->id }}" action="{{ route('discussions.comments.delete', ['comment' => $comment]) }}" method="post">
                    @method('delete')
                    @csrf
                </form>
            </div>
