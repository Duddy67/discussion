    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="true">
          ...
          @allowto('create-discussions')
              @php $open = (request()->is('admin/discussions*')) ? 'menu-open' : '' @endphp
              <li class="nav-item {{ $open }}">
                  @php $active = (request()->is('admin/discussions*')) ? 'active' : '' @endphp
                  <a href="#" class="nav-link {{ $active }}">
                    <i class="nav-icon fas fa-pencil-alt"></i>
                    <p>@lang ('labels.title.discussions')<i class="right fas fa-angle-left"></i></p>
                  </a>
                  <ul class="nav nav-treeview">
                          @php $active = ($routeName == 'admin.discussions.index' || $routeName == 'admin.discussions.create' || $routeName == 'admin.discussions.edit') ? true : false @endphp
                          <x-menu-item href="{{ route('admin.discussions.index') }}" :sub=true :active="$active">
                            @lang ('labels.title.discussions')
                          </x-menu-item>
                      @allowto('create-discussion-categories')
                          @php $active = (request()->is('admin/discussions/categories*')) ? true : false @endphp
                          <x-menu-item href="{{ route('admin.discussions.categories.index') }}" :sub=true :active="$active">
                            @lang ('labels.title.categories')
                          </x-menu-item>
                      @endallowto
                      @allowto('discussion-settings')
                          @php $active = (request()->is('admin/discussions/settings*')) ? true : false @endphp
                          <x-menu-item href="{{ route('admin.discussions.settings.index') }}" :sub=true :active="$active">
